...
3. **`ContactPerson`** — Sheet 02. *Shipped 3 Sep.* Individual people at an
   ...
   see ARCHITECTURE.md.
4. Follow-ups surfaced in the existing `/alerts` engine. *Still not built
   as of 2026-09-02 — items 1-3 above have all merged to `main`
   (`crm.prisma` carries both `ContactInteraction` and `ContactPerson`,
   `lib/actions/crm.ts` is on main), and this one has not started:
   `lib/alerts.ts` and `lib/alerts-query.ts` contain no reference to
   `ContactInteraction` or to a follow-up date. It is the only unfinished
   item in Phase A, and therefore the only thing standing between this
   lane and Phase B.*
