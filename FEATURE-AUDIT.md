...
| Status | Feature | Note |
| --- | --- | --- |
| Built | Company profile with registered legal entity per state (multi-state licensing) | `CompanyLicense` + `LicenseClassificationReference`, seeded for CA/AZ/UT. The company's OWN identity — legal name, DBA, EIN, HQ address, phone, website — is editable on `/settings` since 5 Sep. **Before that there was no `prisma.company.update` anywhere in `apps/web`:** the name was generated at first sign-in as `"<your name>'s Company"` and was permanent, and `dbaName`/`ein`/`hqAddress*`/`phone`/`website` had been in the schema with nothing that could write them. Renaming does not alter an already-signed contract — `SignatureRequest.snapshot` is a copy taken at signing, and `company.test.ts` pins that no rename writes to it |
| Built | Trade-scope tags per company (multi-select, not single trade) | `CompanyTradeScope` join table, one row per held scope — with add/edit/remove on `/settings` since 5 Sep, and `isPrimary` enforced as exactly one. **It was marked Built on the model alone from 24 Aug until then, during which the table had ZERO references anywhere in `apps/web`** — no read, no write, no page. Third row on this sheet with that history, after the licence row and the union row, and found the same way both of those were: by writing the click-list and discovering step one was impossible |
