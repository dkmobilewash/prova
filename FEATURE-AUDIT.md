...
## 03. Estimating & Bidding — 18 built · 0 partial · 0 missing
...
| Built | A bid carries its alternates, unit prices and allowances, not just one number | `BidLine` (`bid-lines.prisma`) + `lib/bid-lines.ts` (pure, 17 tests), shown per bid on `/bids`. A GC's bid form asks for a base, then alternates it may or may not take, then unit prices it will hold you to, and sometimes an allowance for undrawn scope; before this they lived in `notes` or in the estimator's head. **The three kinds sit in three different places relative to the base and the totals keep them there**: an ALLOWANCE is already INSIDE the base (reported, never added — adding it sends the bid out high by exactly the allowance), an ALTERNATE is outside it and counts only once accepted, and a UNIT_PRICE is a rate with no `amount` column at all so no careless sum can pick it up. An alternate's amount is SIGNED — one column, because an amount plus a direction flag can disagree and a deduct stored positive is wrong by twice the alternate — and the direction is printed in words beside it, since a minus sign is the easiest thing on a bid document to miss. `accepted` has three states: "they have not said" is a different fact from "declined", and the screen calls a total provisional while any alternate is unanswered |
| Built | A won bid is linked to the job it became, and compared to what that job cost | `BidInvitation.wonJobId` (nullable, SET NULL, unique) + `lib/bid-outcome.ts` (pure, 17 tests). The schema said for months that "linking a won BidInvitation forward to its Job is a future refinement, not built here", and without it the app could not answer the one question an estimator most needs: did we bid this right. Every other feedback loop is narrower — `catalog-actuals` per catalog entry, `phase-code-rollup` per phase code, `bid-pipeline` win/loss. **A verdict is only given on a FINISHED job**: while it runs, cost to date is reported as cost to date with the percent complete beside it and the word margin never appears, because "you bid this 40% over" from a job three weeks in is a real-looking number that gets remembered. The company-level record averages only settled jobs and states how many were excluded, rather than counting unfinished work as on-budget. Cost comes from `calculateJobWip`, the same definition the WIP schedule and the job costing panel use — never a second sum. The link is picked by a PERSON: names rarely match the GC's wording and a fuzzy match would judge a bid against another job's costs, the same posture `sourceCatalogEntryId` takes |
