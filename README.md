...
| `ANTHROPIC_API_KEY` | `apps/web/.env` | From your Anthropic Console — required for every AI feature: compliance document extraction, draft-estimate-from-text, WIP summaries, and the dashboard's Ask box (both its answers and its commands). Without it, those features fail at request time rather than at startup; Ask says so in its own words. Set it on Vercel Preview as well as Production, or nothing about Ask can be clicked on a preview. |
