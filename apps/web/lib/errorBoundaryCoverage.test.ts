...
function productionError(): Error & { digest?: string } {
  const error: Error & { digest?: string } = new Error(
    "An error occurred in the Server Components render. The specific message is omitted in production builds",
  );
  error.digest = "446730191";
  return error;
}

async function renderBoundary(load: () => Promise<{ default: Boundary }>): Promise<string> {
  const { default: Boundary } = await load();
  return renderToStaticMarkup(createElement(Boundary, { error: productionError(), reset: () => {} }));
}
...
describe("all three boundaries render the same screen", () => {
  ...
  it.each(BOUNDARIES)("%s names the Migrate demo database workflow on a PREVIEW", async (_name, load) => {
    vi.stubEnv("NEXT_PUBLIC_DEPLOY_ENV", "preview");
    const html = await renderBoundary(load);
    expect(html).toContain("Migrate demo database");
    expect(html).toContain("usually the database, not the code");
  });

  it.each(BOUNDARIES)("%s says nothing about the demo database on PRODUCTION", async (_name, load) => {
    vi.stubEnv("NEXT_PUBLIC_DEPLOY_ENV", "production");
    const html = await renderBoundary(load);
    // On production that cause is impossible (CI migrates on merge), so
    // naming it would point a real user at the wrong thing.
