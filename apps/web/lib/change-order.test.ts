 ...
 * lib/change-order.ts was the only calculation module in lib/ without one,
 * which is how a declared-and-never-read isDeleted field survived in it.
 */
...
describe("proposalValueDelta", () => {
  ...
  it("subtracts what a removed line is worth right now", () => {
    ...
    expect(
      proposalValueDelta(proposal({ changeType: "REMOVE" }), line()).toString(),
    ).toBe("-5000");
  });
  ...
  it("prices an edit as the difference, not the new value", () => {
    ...
    expect(
      proposalValueDelta(proposal({ quantity: d(120) }), line()).toString(),
    ).toBe("1000");
  });
});

describe("scope another change order already removed", () => {
  // The defect: LineItemForChangeOrder declared isDeleted and nothing ever
  // read it. A pending REMOVE of a line an earlier approved change order
  // had already soft-deleted reported −$5,000 of exposure — money the
  // headline asked the GC for that could never be booked, because
  // approveChangeOrder refuses that proposal outright.
  ...
  it("still counts an APPLIED removal, which is the whole point of the snapshot", () => {
    ...
    // snapshot is the record of what it was worth when it went, and
    // zeroing this one would erase an approved change order's own effect.
  ...
  it("keeps the pending exposure to money that can actually be booked", () => {
    ...
    // with one proposal flagged — not −$2,000.
    const targets = new Map([["line-1", removedLine]]);
    const changeOrders = [
      {
        status: "SUBMITTED",
...
describe("laterApprovedConflict", () => {
  ...
  it("names every later change order once, in order", () => {
  ...
  it("refuses when the order cannot be established", () => {
    ...
    // allow silently rewrites a contract value.
