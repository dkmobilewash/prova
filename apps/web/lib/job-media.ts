 ...
 *
 * `isBlobStorageUrl`, `blobStoreId` and `isOurBlobStoreUrl` USED TO LIVE
 * HERE and now live in lib/blob-urls.ts, unchanged. Nothing about them was
 * ever about photographs — they are about the store — and the five document
 * uploads (#27) needed the same three checks, so they have a shared home
 * rather than a second copy. Importers that want "is this our store's URL"
 * import that module directly; this one imports it for its own use below.
 */
