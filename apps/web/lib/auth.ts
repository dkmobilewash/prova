...
export async function requireCompanyContext() {
  ...
  try {
    ...
    //
    // THE SAME DEAD-GUARD SHAPE AS #25 AND #26, found while fixing them and
    // in neither issue. This read `error instanceof
    // Prisma.PrismaClientKnownRequestError`, which is false at runtime under
    // Next's bundling — so the whole recovery below was unreachable and a
    // concurrent first sign-in 500'd with a raw Prisma error rather than
    // re-reading what the winner created.
    //
    // Narrower than what it replaces, deliberately. The original admitted
    // ANY known request error; the race it describes is a unique-constraint
    // collision on `clerkId` or `email`, which is P2002 and nothing else.
    // Any other Prisma failure re-reading the user is a genuine bug and
    // should keep escaping rather than being silently retried.
    //
    // Note this is NOT the adoption path CLAUDE.md describes — that one runs
    // above, before the try, and was never affected.
