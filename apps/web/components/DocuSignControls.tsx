...
import { disconnectDocuSign } from "@/lib/actions";
...
/**
 * Connect / Reconnect / Disconnect for the DocuSign card — the Jobber card's
 * arrangement. Connect is a LINK (it leaves the app for DocuSign's consent
 * screen, and /api/docusign/start sets the state cookie on that redirect).
 * Disconnect is the shared two-step control, never `window.confirm`.
 */
export function DocuSignControls({ state, startHref }: { state: DocuSignCardState; startHref: string }) {
  const [isPending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);

  if (state === "not-set-up") {
    return (
      <p className="max-w-[16rem] text-right text-xs text-ink-muted" data-tour="docusign-not-set-up">
        Not set up on this install yet. Whoever runs C Stream for you has to add the DocuSign app keys
  ...
  if (state === "connect" || state === "reconnect") {
    return (
      <div className="flex flex-col items-end gap-2">
        <a href={startHref} className={linkButton} data-tour="docusign-connect">
          {state === "reconnect" ? "Reconnect" : "Connect"}
        </a>
        <span className="max-w-[16rem] text-right text-xs text-ink-muted">
          Opens DocuSign to sign in and allow C Stream to send envelopes from your account.
        </span>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-end gap-2">
      <RowActions
        ...
        destructive={
          <ConfirmDelete
            describe="C Stream stops sending through DocuSign. Envelopes already sent stay on their jobs and keep going to the signers."
            label="Disconnect"
            ...
            onConfirm={() => {
              ...
              startTransition(async () => {
                const result = await disconnectDocuSign();
            ...
            hint={
              <span className="max-w-[16rem] text-right text-ink-muted">
                Envelopes already sent are kept.
              </span>
            }
