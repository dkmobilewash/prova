...
export default async function IntakePage() {
  ...
  const [jobs, tray, trayTotal, filed, dismissed] = await Promise.all([
    prisma.job.findMany({
      ...
      // status and the GC's name are not decoration: `jobPickerLabel` needs
      // them, and `toJobOption` will not accept a row without them. Issue
      // #65 — fifteen jobs, seven of them called the same thing.
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      {rows.length === 0 ? (
        ...
        <>
          <IntakeTable rows={rows} jobs={jobs.map(toJobOption)} />
          {withheld > 0 && (
