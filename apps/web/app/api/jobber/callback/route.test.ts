...
import { JOBBER_OAUTH_STATE_COOKIE } from "@/lib/jobber/constants";

/**
 * The Jobber OAuth callback binds the connection to the SIGNED-IN company,
...
vi.mock("@prova/integrations", () => ({
  readJobberConfig: () => ({ clientId: "cid", clientSecret: "cs", redirectUri: "https://app.cstream.ai/api/jobber/callback" }),
  exchangeJobberCode: async (_config: unknown, code: string, verifier: string) => {
    exchanged.push({ code, verifier });
    return { accessToken: "jobber-access-PLAINTEXT", refreshToken: "jobber-refresh-PLAINTEXT" };
  },
  fetchJobberAccount: async () => ({ id: "acct_1", name: "Smith Remodeling" }),
}));
...
function callback(state = STATE, extra = "") {
  return new NextRequest(`https://app.cstream.ai/api/jobber/callback?code=jobber-code&state=${state}${extra}`);
}

function outcome(response: Response) {
  const location = new URL(response.headers.get("location") ?? "", "https://app.cstream.ai");
  return { path: location.pathname, jobber: location.searchParams.get("jobber"), detail: location.searchParams.get("jobber_detail") };
}
...
describe("Jobber OAuth callback", () => {
  it("refuses a state that doesn't match the cookie — nothing exchanged, nothing written", async () => {
    cookie({ state: "b".repeat(48) });
    expect(outcome(await GET(callback()))).toEqual({ path: "/settings/integrations", jobber: "error", detail: "state_mismatch" });
  ...
  it("connects the SESSION's company, with the PKCE verifier, storing only encrypted tokens", async () => {
    cookie();
    expect(outcome(await GET(callback()))).toEqual({ path: "/settings/integrations", jobber: "connected", detail: null });
    expect(exchanged).toEqual([{ code: "jobber-code", verifier: "v".repeat(64) }]);
    expect(written).toHaveLength(1);
    expect(written[0].where).toEqual({ companyId_provider: { companyId: "co_A", provider: "JOBBER" } });
    const stored = JSON.stringify(written[0]);
    expect(stored).not.toContain("PLAINTEXT");
    expect(written[0].create).toEqual(
      expect.objectContaining({ companyId: "co_A", status: "CONNECTED", externalAccountLabel: "Smith Remodeling" }),
    );
    expect(String(written[0].create.encryptedAccessToken).startsWith("v1.")).toBe(true);
    expect(logs).toHaveLength(1);
    // The cookie is single-use.
    expect(cookieJar.has(JOBBER_OAUTH_STATE_COOKIE)).toBe(false);
  });
